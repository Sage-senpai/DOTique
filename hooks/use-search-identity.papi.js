...
import { extractText, hasPositiveIdentityJudgement, } from "@/lib/utils.dot-ui";
...
export function useIdentitySearch(displayName, identityChain = "paseoPeople") {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            if (!hasIdentityPallet(peopleApi) ||
                !displayName ||
            ...
            try {
                ...
                for (const { keyArgs, value } of entries) {
                    if (!value || !value.info?.display?.value)
                        continue;
        ...
        enabled: hasIdentityPallet(peopleApi) &&
            !!displayName &&
            displayName.length >= 3 &&
            isConnected,
